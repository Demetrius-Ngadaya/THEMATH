...
export default function Navbar() {
    ...
    return (
        <>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex h-20 items-center justify-between">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            {/* Wishlist - Only show count after mounted to avoid hydration error */}
                            ...
                            {/* Cart - Only show count after mounted to avoid hydration error */}
                            <Link href="/cart" className="relative rounded-full p-2 text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors">
                                ...
                                {mounted && cartCount > 0 && (
                                    <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-blue-600 text-xs text-white">
                                        {cartCount}
                                    </motion.span>
                                )}
                            </Link>
                            ...
                            {isAuthenticated ? (
                                ...
                                <Link href="/login" className="rounded-full p-2 text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors">
                                    <HiOutlineUser className="h-5 w-5" />
                                </Link>
                            )}

                            {/* Mobile Menu Button */}
                            <button onClick={() => setIsOpen(!isOpen)} className="rounded-full p-2 text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors lg:hidden">
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: "100%" }} transition={{ type: "tween" }} className="fixed inset-y-0 right-0 z-40 w-64 bg-white shadow-xl dark:bg-gray-900 lg:hidden">
                        <div className="flex h-full flex-col p-6">
                            <div className="flex-1 space-y-4">
                                {navLinks.map((link) => (
                                    <Link key={link.href} href={link.href} onClick={() => setIsOpen(false)} className={`block text-lg font-medium ${pathname === link.href ? "text-blue-600 dark:text-blue-400" : "text-gray-700 dark:text-gray-300"
                                        }`}>
                                        {link.label}
                                    </Link>
                                ))}
                                {isAuthenticated && (
                                    <Link href="/orders" onClick={() => setIsOpen(false)} className="block text-lg font-medium text-gray-700 dark:text-gray-300">
                                        My Orders
                                    </Link>
                                )}
                            </div>
                            <div className="border-t border-gray-200 pt-4 dark:border-gray-800">
                                {isAuthenticated ? (
                                    <button onClick={handleLogout} className="flex w-full items-center space-x-2 text-red-600">
                                        <HiOutlineLogout className="h-5 w-5" />
                                        <span>Logout</span>
                                    </button>
                                ) : (
                                    <Link href="/login" onClick={() => setIsOpen(false)} className="flex items-center space-x-2 text-gray-700 dark:text-gray-300">
                                        <HiOutlineUser className="h-5 w-5" />
                                        <span>Sign In</span>
                                    </Link>
                                )}
